...
export function getChaiOrderLength(order) {
  ...
  const finalString = order.trim().length;
  return finalString
}
...
export function hasSpecialIngredient(order, ingredient) {
  ...
  const orderString = order.trim().toLowerCase();
  const ingredientString = ingredient.trim().toLowerCase();
  const checkIngredient = orderString.includes(ingredientString);
