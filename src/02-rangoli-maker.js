...
export function repeatPattern(pattern, times) {
  if (times < 1 || typeof pattern != 'string' || !Number.isInteger(times) || times < 1) return ""
  const finalPattern = pattern.repeat(times);
  return finalPattern
}
...
export function replaceRangoliColor(design, oldColor, newColor) {
  ...
  const finalDesign = design.replaceAll(oldColor, newColor);
  return finalDesign;
}

export function makeRangoliBorder(char, length) {
  if (typeof char !== 'string' || typeof length !== 'number' || !Number.isInteger(length) || length < 1) return "";
  const repeatTimes = Math.ceil(length / char.length);
  const finalString = char.repeat(repeatTimes).slice(0, length);
  return finalString;
}